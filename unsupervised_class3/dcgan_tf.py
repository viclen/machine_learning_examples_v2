...
class DCGAN:
  def __init__(self, img_length, num_colors, d_sizes, g_sizes):
    ...
    self.batch_sz = tf.compat.v1.placeholder(tf.int32, shape=(), name='batch_sz')
    ...
    self.d_cost = tf.reduce_mean(input_tensor=self.d_cost_real) + tf.reduce_mean(input_tensor=self.d_cost_fake)
    ...
    num_correct = tf.reduce_sum(input_tensor=real_predictions) + tf.reduce_sum(input_tensor=fake_predictions)
    self.d_accuracy = num_correct / num_predictions


    # optimizers
    self.d_params = [t for t in tf.compat.v1.trainable_variables() if t.name.startswith('d')]
    self.g_params = [t for t in tf.compat.v1.trainable_variables() if t.name.startswith('g')]

    self.d_train_op = tf.compat.v1.train.AdamOptimizer(
      LEARNING_RATE, beta1=BETA1
    ).minimize(
      self.d_cost, var_list=self.d_params
    )
    self.g_train_op = tf.compat.v1.train.AdamOptimizer(
      LEARNING_RATE, beta1=BETA1
    ).minimize(
