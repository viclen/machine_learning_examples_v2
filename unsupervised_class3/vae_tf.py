...
class VariationalAutoencoder:
  def __init__(self, D, hidden_layer_sizes):
    ...
    if st is None:
      ...
      e = standard_normal.sample(tf.shape(input=self.means)[0])
    ...
    self.Z_input = tf.compat.v1.placeholder(tf.float32, shape=(None, M))
    ...
    # now build the cost
    if st is None:
      kl = -tf.math.log(self.stddev) + 0.5*(self.stddev**2 + self.means**2) - 0.5
      kl = tf.reduce_sum(input_tensor=kl, axis=1)
    else:
      kl = tf.reduce_sum(
        input_tensor=tf.contrib.distributions.kl_divergence(
          self.Z.distribution, standard_normal
        ),
        axis=1
      )
    expected_log_likelihood = tf.reduce_sum(
      input_tensor=self.X_hat_distribution.log_prob(self.X),
      axis=1
    )
    ...
    self.elbo = tf.reduce_sum(input_tensor=expected_log_likelihood - kl)
    self.train_op = tf.compat.v1.train.RMSPropOptimizer(learning_rate=0.001).minimize(-self.elbo)
