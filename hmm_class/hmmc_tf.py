...
class HMM:
    ...
    def build(self, preSoftmaxPi, preSoftmaxA, preSoftmaxR, mu, logSigma):
        ...
        # X will be TxD
        self.tfx = tf.compat.v1.placeholder(tf.float32, shape=(None, self.D), name='X')
        ...
        def recurrence(old_a_old_s, B_t):
            ...
            s = tf.reduce_sum(input_tensor=a)
            return (a / s), s
        ...
        self.cost_op = -tf.reduce_sum(input_tensor=tf.math.log(scale))
        self.train_op = tf.compat.v1.train.AdamOptimizer(1e-2).minimize(self.cost_op)
