...
class SimpleRNN:
    ...
    def build(self, We, Wx, Wh, bh, h0, Wo, bo):
        ...
        # placeholders
        self.tfX = tf.compat.v1.placeholder(tf.int32, shape=(None,), name='X')
        self.tfY = tf.compat.v1.placeholder(tf.int32, shape=(None,), name='Y')
        ...
        # X_one_hot.dot(We)
        XW = tf.nn.embedding_lookup(params=We, ids=self.tfX)
        ...
        nce_weights = tf.transpose(a=self.Wo, perm=[1,0]) # needs to be VxD, not DxV
        nce_biases = self.bo
