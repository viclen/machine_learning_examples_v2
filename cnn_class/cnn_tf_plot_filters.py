...
def convpool(X, W, b):
    ...
    pool_out = tf.nn.max_pool2d(input=conv_out, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
...
) + 0.01*sum(tf.reduce_sum(input_tensor=p*p) for p in params)
...
predict_op = tf.argmax(input=Yish, axis=1)
