...
class TFLogistic:
  ...
  def build(self, D, K):
    ...
    self.inputs = tf.compat.v1.placeholder(tf.float32, shape=(None, D), name='inputs')
    self.targets = tf.compat.v1.placeholder(tf.int64, shape=(None,), name='targets')
    ...
    self.saver = tf.compat.v1.train.Saver({'W': self.W, 'b': self.b})
    ...
    self.predict_op = tf.argmax(input=logits, axis=1)
    return cost
  ...
  def fit(self, X, Y, Xtest, Ytest):
    ...
    cost = self.build(D, K)
    l2_penalty = regularization*tf.reduce_mean(input_tensor=self.W**2) / 2
    cost += l2_penalty
    train_op = tf.compat.v1.train.MomentumOptimizer(lr, momentum=mu).minimize(cost)
