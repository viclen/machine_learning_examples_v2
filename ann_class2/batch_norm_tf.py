...
class HiddenLayerBatchNorm(object):
  ...
  def forward(self, X, is_training, decay=0.9):
    ...
    if is_training:
      batch_mean, batch_var = tf.nn.moments(x=activation, axes=[0])
      update_running_mean = tf.compat.v1.assign(
        self.running_mean,
        self.running_mean * decay + batch_mean * (1 - decay)
      )
      update_running_var = tf.compat.v1.assign(
        self.running_var,
...
class ANN(object):
  ...
  def fit(self, X, Y, Xtest, Ytest, activation=tf.nn.relu, learning_rate=1e-2, epochs=15, batch_sz=100, print_period=100, show_fig=True):
    ...
    self.predict_op = tf.argmax(input=test_logits, axis=1)
