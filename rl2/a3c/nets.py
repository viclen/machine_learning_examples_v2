...
class PolicyNetwork:
  def __init__(self, num_outputs, reg=0.01):
    ...
    self.states = tf.compat.v1.placeholder(shape=[None, 84, 84, 4], dtype=tf.uint8, name="X")
    # Advantage = G - V(s)
    self.advantage = tf.compat.v1.placeholder(shape=[None], dtype=tf.float32, name="y")
    # Selected actions
    ...
    with tf.compat.v1.variable_scope("policy_network"):
      ...
      # Sample an action
      cdist = tf.compat.v1.distributions.Categorical(logits=self.logits)
      self.sample_action = cdist.sample()

      # Add regularization to increase exploration
      self.entropy = -tf.reduce_sum(input_tensor=self.probs * tf.math.log(self.probs), axis=1)

      # Get the predictions for the chosen actions only
      batch_size = tf.shape(input=self.states)[0]
      gather_indices = tf.range(batch_size) * tf.shape(input=self.probs)[1] + self.actions
      self.selected_action_probs = tf.gather(tf.reshape(self.probs, [-1]), gather_indices)

      self.loss = tf.math.log(self.selected_action_probs) * self.advantage + reg * self.entropy
      self.loss = -tf.reduce_sum(input_tensor=self.loss, name="loss")
  ...
  def __init__(self):
    ...
    with tf.compat.v1.variable_scope("value_network"):
      self.vhat = tf.contrib.layers.fully_connected(
        ...
        num_outputs=1,
        activation_fn=None)
      self.vhat = tf.squeeze(self.vhat, axis=[1], name="vhat")

      self.loss = tf.math.squared_difference(self.vhat, self.targets)
      self.loss = tf.reduce_sum(input_tensor=self.loss, name="loss")
